...
export default function CallsScreen() {
  ...
  const renderItem = ({ item }: { item: CallRow }) => {
    ...
    const peer = contactsMap[peerId];
    const displayName =
      peer?.name ||
      (peerId === item.caller_id ? item.caller_name : null) ||
      (peer?.username ? `@${peer.username}` : 'Unknown');
    const direction = item.direction;
    const isMissed = direction === 'incoming' && !item.answered_at;
