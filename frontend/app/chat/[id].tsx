...
export default function ChatScreen() {
  ...
  // Tick every second to update countdown badges and auto-prune expired messages
  useEffect(() => {
  ...
  // Polling fallback (5 s) — covers WebSocket reconnect gaps
  useEffect(() => {
    if (!id) return;
  ...
  const sendMessage = async (
    ...
    try {
      ...
      if ((kind === 'text' || e2eeAttachment) && conv && user?.id && isConversationE2EEReady(conv, user.id)) {
  ...
  const uploadForCurrentConversation = async (
    ...
    return { upload: await uploadCandidate(candidate), e2eeAttachment: null };
  };
