...
export type E2EECallSignalPayload = {
  version: 1;
  algorithm: Algorithm;
  sender_public_key: string;
  nonce: string;
...
export async function encryptCallSignalForUser(
  signal: Record<string, unknown>,
  peerPublicKey: string,
  currentUserId: string,
): Promise<E2EECallSignalPayload> {
  const identity = await ensureE2EEIdentity(currentUserId);
  await api.post('/e2ee/keys', {
    public_key: identity.publicKey,
    algorithm: identity.algorithm,
  ...
  const nonce = await Crypto.getRandomBytesAsync(nacl.box.nonceLength);
  const ciphertext = nacl.box(
    utf8ToBytes(JSON.stringify(signal)),
    nonce,
...
export async function decryptCallSignalFromUser(
  payload: E2EECallSignalPayload | null | undefined,
  expectedSenderPublicKey: string | null | undefined,
  currentUserId: string | null | undefined,
): Promise<Record<string, unknown> | null> {
  if (!payload || payload.algorithm !== ALGORITHM || !currentUserId) return null;
  ...
  try {
    const opened = nacl.box.open(
      ...
      base64ToBytes(payload.nonce),
      base64ToBytes(payload.sender_public_key),
      base64ToBytes(identity.secretKey),
    );
    if (!opened) return null;
    const parsed = JSON.parse(bytesToUtf8(opened));
