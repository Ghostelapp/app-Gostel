 ...
 *
 * `react-native-callkeep` exposes the Android Telecom / iOS CallKit native
 * incoming-call UI. We use it ONLY for the wakeup + native call screen; the
 * actual media transport stays on our existing WebRTC stack (/app/call/[id]).
 *
 * Lifecycle:
 *   1. setupCallKeep() runs once at app boot (called from _layout.tsx).
 *   2. FCM background handler (src/fcmBackground.ts) calls
 ...
 *
 * iOS support is currently a no-op (returns early on iOS) — Phase J2.
 */
...
export async function setupCallKeep(): Promise<boolean> {
  ...
  try {
    ...
    await RNCallKeep.setup({
      ...
      android: {
        alertTitle: 'Wymagane uprawnienia',
        alertDescription:
          'Aby odbierać połączenia, Ghostel potrzebuje dostępu do funkcji telefonu.',
        cancelButton: 'Anuluj',
        okButton: 'Zezwól',
        // selfManaged=false → use the native Telecom UI (NOT our own).
        selfManaged: false,
        // Foreground service notification shown while a call is active.
        foregroundService: {
          channelId: 'ghostel.calls.fg',
          channelName: 'Aktywne połączenie',
          notificationTitle: 'Połączenie Ghostel',
          notificationIcon: 'ic_notification',
        },
        additionalPermissions: ['android.permission.READ_PHONE_STATE'],
      },
    });
    // Tell Android that we're ready to receive incoming calls.
    try {
